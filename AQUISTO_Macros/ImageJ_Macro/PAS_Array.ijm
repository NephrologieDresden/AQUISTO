// Macro to set the parameters for the processing and thresholding of pas stained sections
...
function secondRun(){
	...
	for(a=0; a<files.length; a++){
		if(a==0){
			newImage("Original", "RGB black", count*1025,1000,1);
			newImage("Processed", "8-bit black", count*1025, 1000,1);
			newImage("Segmented", "8-bit black", count*1025, 1000,1);
		}
		run("Set Measurements...", "median standard"); 
		open(path+"Macros\\Samples\\Channels\\"+files[a]);
		...
		run("Crop");
		run("Canvas Size...", "width=1025 height=1025 position=Center");
		...
		run("Insert...", "source=OriginalImage destination=Original x="+a*1025+" y=0");

		selectWindow("SegmentedImage");
		run("Insert...", "source=SegmentedImage destination=Segmented x="+a*1025+" y=0");
			
		selectWindow("ProcessedImage");
		drawString("Median="+medianprocessed+", Std="+sdprocessed, 100, 100, "white");
		run("Insert...", "source=ProcessedImage destination=Processed x="+a*1025+" y=0");
...
if(second==1){
	run("Clear Results");
	parameters=newArray("HistAd","Background", "Median", "Gauss", "Method", "Threshold");
	values=newArray(histad, background, median, gauss, method, threshold);
	for(a=0; a<parameters.length; a++){
		setResult("Parameters", a, parameters[a]);
		setResult("Values", a, values[a]);
	}
	saveAs("Results", path+"Macros\\Processing\\PAS.txt");
}
